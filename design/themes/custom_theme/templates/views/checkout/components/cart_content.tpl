                                        ...
                                        {hook name="checkout:items_list"}
                                            ...
                                            {if !$cart.products.$key.extra.parent}
                                                <tr>
                                                    ...
                                                    <td class="shoping__cart__price cm-reload-{$obj_id}" id="price_display_update_{$obj_id}">
                                                    ...
                                                    <td class="shoping__cart__quantity ty-cart-content__qty {if $product.is_edp == "Y" || $product.exclude_from_calculate} quantity-disabled{/if}">
                                                    ...
                                                    <td class="shoping__cart__total  cm-reload-{$obj_id}" id="price_subtotal_update_{$obj_id}">
                                                    ...
                                                    {strip}
                                                            ...
                                                            {if !$product.exclude_from_calculate}
                                                                <a class="{$ajax_class} ty-cart-content__product-delete ty-delete-big" href="{"checkout.delete?cart_id=`$key`&redirect_mode=`$runtime.mode`"|fn_url}" data-ca-target-id="cart_items,checkout_totals,cart_status*,checkout_steps,checkout_cart" title="{__("remove")}"><span class="icon_close"></span></a>
                                                            {/if}
                                                        {/strip}
...
<div class="row">
                ...
                <div class="col-lg-6">
                    <div class="shoping__checkout">
                        <h5>Cart Total</h5>
                        <ul>
                            <li>{__("subtotal")} <span>{include file="common/price.tpl" value=$cart.display_subtotal}</span></li>
                                    ...
                                    {if $cart.shipping}
                                     {foreach $cart.shipping as $shipping_id => $shipping name="f_shipp"}
                                            ...
                                            {if $show_shipping_estimation}
                                               ({$smarty.capture.shipping_estimation|trim nofilter})
                                            {/if}
                                      
                                        
                                    {elseif $show_shipping_estimation}
                                        {__("shipping_cost")}
                                        <span class="">{$smarty.capture.shipping_estimation nofilter}</span>
                                    {/if}
                            ...
                            <li class="">
                                ...
                                <span class="">{include file="common/price.tpl" value="-`$cart.discount`"}</span>
                            </li>
                            ...
                            <li class="">
                                 {__("order_discount")}
                                <span class="">{include file="common/price.tpl" value="-`$cart.subtotal_discount`"}</span>
                            </li>
                            ...
                            <li class="">
                                {$tax.description}&nbsp;({include file="common/modifier.tpl" mod_value=$tax.rate_value mod_type=$tax.rate_type}{if $tax.price_includes_tax == "Y" && ($settings.Appearance.cart_prices_w_taxes != "Y" || $settings.Checkout.tax_calculation == "subtotal")}&nbsp;{__("included")}{/if})
                                <span class="">{include file="common/price.tpl" value=$tax.tax_subtotal}</span>
                            </li>
                            {/foreach}
                            {/if}
                            ...
                            <li class="" id="payment_surcharge_line">
                                ...
                                <span class="">{$cart.payment_surcharge_title|default:__("payment_surcharge")}{if $payment_data.payment}&nbsp;({$payment_data.payment}){/if}:</span>
                                <span class="">{include file="common/price.tpl" value=$cart.payment_surcharge span_id="payment_surcharge_value"}</span>
                            </li>
                            ...
                            <li class="">
                                {__("total_cost")}
                                <span>
                                    {include file="common/price.tpl" value=$_total|default:$smarty.capture._total|default:$cart.total span_id="cart_total" class=""}
                                </span>
                            </li>
                        </ul>
