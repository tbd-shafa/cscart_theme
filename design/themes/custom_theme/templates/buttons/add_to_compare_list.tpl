...
{if !$hide_compare_list_button}
    ...
    {if $compare_button_type == "icon"}
        {$but_icon        = $compare_but_icon|default:     ""}
        {$but_text        = $compare_but_text|default:     false}
        {$but_meta        = $compare_but_meta|default:     "ty-btn__tertiary ty-btn-icon ty-add-to-wish heart-icon fa fa-retweet icon_heart_alt $ajax_class"}
    {else}
        {$but_icon        = ($compare_but_icon === true) ? "" : $compare_but_icon}
